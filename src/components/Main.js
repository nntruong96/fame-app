...
const PURESTAKE_API_PORT = '',
  PURESTAKE_API_KEY = 'PSNpXRUE5G5kL998LKDd43Xfzl7Q2YBo5li8hBzw';
export default class App extends React.Component {
  ...
  handleSendFunds = event => {
    Algo.sendFunds(
      'EF64SJ6HMEK4FAKC3HI5NA76JMUNOWPVYZF3U7NUYPYQ5VMIE2QZZE5NCM',
      'hi',
      10,
      (error, result) => {
        if (error) {
  ...
  handleCreateMultisigAddress = event => {
    Algo.createMultiSignatureAddress(
      1,
      2,
      [
  ...
  handleCreateMultisigTransaction = event => {
    Algo.createMultisigTransaction(
      'EF64SJ6HMEK4FAKC3HI5NA76JMUNOWPVYZF3U7NUYPYQ5VMIE2QZZE5NCM',
      'hi',
      11,
      (error, result) => {
  ...
  handleApproveMultisigTransaction = event => {
    Algo.recoverAccount(
      ...
      (error, result) => {
        ...
        Algo.approveMultiSigTransaction(
          this.state.signedTrans,
          (error, result) => {
            if (error) {
              console.error(error);
              return;
  ...
  handleCreateAndGroupTransaction = () => {
    ...
    let {accountInfo} = this.state;
    Algo.createTransactionForGrouping(
      'V7QKGG7N2N2XAEH7FSLHEWEJ2YYMKSTKQE2A2WIPM2BPROKCIQNSXYAJUM',
      'Hi there',
      1,
      accountInfo.publicAddress,
    ...
    if (node == 'hackathon') {
      ...
      Algo.createClientFromHackathonInstance((error, result) => {
        if (error) {
          ...
          console.log('error');
          this.setState({
            connectToNodeData: error,
            connectToNodeBanner: true,
            connectToNode: false,
          });
          return;
        }
      ...
      if (network == 'testnet') {
        ...
        Algo.createClientFromPurestake(
          'TESTNET',
          443,
          'ADRySlL0NK5trzqZGAE3q1xxIqlQdSfk1nbHxTNe',
          (error, result) => {
            if (error) {
      ...
      Algo.createClientFromSandbox(
        customNodeAddress,
        parseInt(customNodePort),
        customNodeToken,
        (error, result) => {
          if (error) {
            console.error(error);
  ...
  handleRecoverAccountClicked = () => {
    ...
    Algo.recoverAccount(seedphraseInput.trim(), (error, result) => {
      ...
      console.log(result);
      this.setState({
        recoverAccountData: `Address : ${result.publicAddress}\n Mnemonic : ${result.mnemonic}`,
        recoverAcccountBanner: true,
      });
    });
  };
  ...
  handleCreateAccountClicked = () => {
    Algo.createNewAccount(result => {
      this.setState({
        createAccountData: `Address : ${result.publicAddress}\n Mnemonic : ${result.mnemonic}`,
      });
      console.log(result);
    });
  };
  ...
  handleGetAccountBalanceClicked = () => {
    ...
    Algo.getAccountBalance(accountBalanceAddress, (error, result) => {
      if (error) {
        console.error(error);
        this.setState({
          accountBalanceData: error,
          accountBalanceLoader: false,
          accountBalanceBanner: true,
        });
        return;
      }
      console.log(result);
      this.setState({
        accountBalanceData: `${result} Algos`,
        accountBalanceLoader: false,
        accountBalanceBanner: true,
      });
    });
    ...
    Algo.sendFunds(
      ...
      (error, result) => {
        if (error) {
          console.error(error);
          this.setState({
            transferFundsData: error,
            transferFundsLoading: false,
            transferFundsBanner: true,
          });
          return;
        }
        this.setState({
          transferFundsData: result,
          transferFundsLoading: false,
          transferFundsBanner: true,
        });
        console.log(result);
        ...
        />
            ...
            {node == 'customnode' ? (
              <View>
                <TextInput
                  ...
                  mode="outlined"
                  onChangeText={text =>
                    this.setState({customNodeAddress: text})
                  }
            ...
            <View style={{alignItems: 'center', marginTop: 15}}>
              <Button
                style={{width: '70%'}}
                mode="contained"
                onPress={this.handleConnectToNodelicked}>
                Connect
              </Button>
            </View>
            <Banner
          ...
          <Card
            ...
            <Card.Title subtitle="Create Account " />
          ...
          <Card
            ...
            <ActivityIndicator
              ...
              animating={accountBalanceLoader}
              color={Colors.black}
            />
            <TextInput
              style={{backgroundColor: '#ffffff'}}
              ...
              mode="outlined"
              onChangeText={text =>
                this.setState({accountBalanceAddress: text})
              }
          ...
          <Card
            ...
            <ActivityIndicator
              style={{position: 'absolute', top: 0, left: 0, right: 0}}
              animating={transferFundsLoading}
              color={Colors.black}
            />
            <TextInput
              style={{backgroundColor: '#ffffff'}}
            ...
            <TouchableOpacity
              style={{marginTop: 10}}
              onPress={this.handleGetAccountBalance}>
              <Text>Get Account Balance</Text>
            </TouchableOpacity>
          </Card>
